...
const createNewUser = async ({ email, password, name, sex, retype }, next) => {
  ...
  if (!(email && name && sex !== null && typeof sex !== 'undefined' && password)) {
    let errors = [];
    ...
    if (sex === null || typeof sex === 'undefined') errors.push({
      code: 'MISSING',
...
const updateInfoUserById = async ({ id, name, sex, address }, next) => {
  ...
  try {
    ...
    const result = await User.findOneAndUpdate({ _id: id }, { name, sex, address }, { new: true });
...
const changePassword = async ({ id, currentPw, newPassword, retype }, next) => {
  if (!(currentPw.length > MIN_LENGTH_PW && newPassword.length > MIN_LENGTH_PW && retype)) {
    let errors = [];
  ...
  try {
    ...
    const result = await User.findOneAndUpdate({ _id: id }, { password: hash }, { new: true });
